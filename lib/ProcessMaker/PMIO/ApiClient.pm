...
#
package ProcessMaker::PMIO::ApiClient;
...
sub _new_instance
{
    ...
    my (%args) = (
        ...
        'base_url' => 'https://CHANGEME.api.processmaker.io/api/v1',
        @_
    );
...
sub _cfg {'ProcessMaker::PMIO::Configuration'}
...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $ProcessMaker::PMIO::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $ProcessMaker::PMIO::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("ProcessMaker::PMIO::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub get_api_key_with_prefix
{
	...
	my $api_key = $ProcessMaker::PMIO::Configuration::api_key->{$key_name};
	
	return unless $api_key;
	
	my $prefix = $ProcessMaker::PMIO::Configuration::api_key_prefix->{$key_name};
...
sub update_params_for_auth {
    ...
    foreach my $auth (@$auth_settings) {
        ...
        elsif ($auth eq 'PasswordGrant') {
            
            if ($ProcessMaker::PMIO::Configuration::access_token) {
                $header_params->{'Authorization'} = 'Bearer ' . $ProcessMaker::PMIO::Configuration::access_token;
            }
        }
        else {
